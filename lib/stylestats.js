...
function StyleStats(args, config) {
  ...
  this.files = [];
  this.styles = [];
  this.options = getOptions(config);

  var EXTENSIONS = ['.css'];
  var excludePathsRegex = createExcludePathsRegex();
  ...
  function getFiles(dir, files) {
    if (excludePathsRegex && dir.match(excludePathsRegex))
      return;
    ...
    innerFiles.forEach(function (file) {
      ...
      if (excludePathsRegex && pathToFile.match(excludePathsRegex))
        return;

      if (EXTENSIONS.indexOf(path.extname(file)) !== -1) {
        files.push(pathToFile);
      }
      else if (util.isDirectory(pathToFile)) {
        getFiles(pathToFile + "\\", files);
      }
    });
  };

  function createExcludePathsRegex() {
    if (!that.options.exclude || !that.options.exclude.length)
      return undefined;

    var escapeRegex = /([.*+?^=!:${}()|\[\]\/\\])/g;

    return new RegExp(that.options.exclude.map(function (p) {
      return p.replace(escapeRegex, "\\$1");
    }).join('|'), 'i');
  }
...
StyleStats.prototype.parse = function (callback) {
  ...
  this.parser.parse(function (error, data) {
    ...
    if (that.options.errors && data.errors) {
