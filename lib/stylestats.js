...
function StyleStats(args, config) {
  ...
  if(this.options.exclude && this.options.exclude.length > 1) {
    excludePathsRegex = new RegExp(this.options.exclude.map(function(p){ return p.replace(/([.*+?^=!:${}()|\[\]\/\\])/g, "\\$1"); }).join('|'), 'i');
  }
