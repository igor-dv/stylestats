...
Analyzer.prototype.analyzeDeclarations = function () {
  ...
  this.declarations.forEach(function (declaration) {
    ...
    // if it contains background-color or border
    if(declaration.property.match(/^background-color$/) || declaration.property.match(/^border/)){
      var rgb = declaration.value.match(/rgba*\(\s*(\d{1,3})\s*,\s*(\d{1,3})\s*,\s*(\d{1,3})\s*,\s*.*\)/);
      var hexColor = declaration.value.match(/#(\w*)/);
      var color;
