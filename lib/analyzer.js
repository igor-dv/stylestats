...
Analyzer.prototype.analyzeDeclarations = function () {
  ...
  this.declarations.forEach(function (declaration) {
    ...
    if(declaration.property.match(colorPropertyRegex)){
      var gradient = declaration.value.match(gradientRegex);
      var rgb = declaration.value.match(rgbRegex);
      var hexColor = declaration.value.match(hexColorRegex);
      var namedColor = declaration.value.match(namedColorsRegex);
      var colors;
      ...
      if(gradient) {
        ...
        colors = [rgbToHex(rgb[1], rgb[2], rgb[3])];
      }
      else if(hexColor){
        colors = ['#' + hexColor[1]];
      }
      else if(namedColor) {
        colors = namedColor.slice(1).filter(function(nc){ return nc; });
      }
