...
Parser.prototype.parse = function (callback) {
  ...
  this.cssFiles.forEach(function (cssFile) {
    ...
    var data = {
      file: cssFile,
      content: fs.readFileSync(cssFile, { encoding: 'utf8' })
    };

    that.filesWithData.push(data);
  });
  ...
  Promise.all(requestPromises).then(function onFulfilled(results) {
    ...
    that.styles.forEach(function(style){
      try {
        rawRules = rawRules.concat(cssParse(style).stylesheet.rules);
        resolvedStyles.push(style);
      } catch (error) {
        parsedData.errors.push('' + error);
      }
    });

    that.filesWithData.forEach(function(data){
      try {
        var parsed = cssParse(data.content);
        var rules = parsed.stylesheet.rules;

        addSourceToRules(rules, fileIndex);

        rawRules = rawRules.concat(rules);
        resolvedStyles.push(data.content);

        var file = getReducedFile(data.file);

        sources[fileIndex] = file;
        fileIndex++;

      } catch (error) {
        parsedData.errors.push(data.file + '. ' + error);
      }
    });

    function addSourceToRules(rules, fileIndex) {
      if (!rules)
        return;

      rules.forEach(function (rule) {
        rule.source = fileIndex;
        addSourceToRules(rule.rules, fileIndex);
      });
    }

    function getReducedFile(file){

      if(!file)
        return file;

      if(that.options.sourceMapFileNameReduce) {
        file = file.replace(that.options.sourceMapFileNameReduce, '');
      }
    ...
    // join all css string
    parsedData.cssString = resolvedStyles.join('');
    parsedData.cssSize = Buffer.byteLength(parsedData.cssString, 'utf8');
    parsedData.sourceMap = { sources: sources, values: {} };
